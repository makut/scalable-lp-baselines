    ...
    def __init__(self, hidden_channels, num_layers, max_z, k=0.6, train_dataset=None,
        ...
        if k <= 1:  # Transform percentile to number.
            if train_dataset is None:
                ...
                if hasattr(train_dataset, 'sample_graphs_for_sortpool'):
                    sample_size = min(len(train_dataset), 1000)
                    sampled_train = train_dataset.sample_graphs_for_sortpool(sample_size)
                elif dynamic_train:
                    sample_size = min(len(train_dataset), 1000)
                    sampled_train = [train_dataset[i] for i in range(sample_size)]
                else:
